...
用户可能在 Plan 阶段确认后发现计划有误并要求修改（"计划不对"、"需求变了"、"改一下方案"）。PjM 处理流程：

1. **识别修改意图**：用户表达对已确认计划的修改需求
2. **重置阶段状态**：
   - resume.md：`phase` 回退至 Plan，`plan_confirmed` 设为 false
   - proposal.md：`plan_confirmed` 设为 false
3. **标记下游影响**：若已存在 design.md，在 resume.md 记录"计划修改后 Design 需重新审视"
4. **分派 PdM**：PdM 根据用户新需求更新 proposal.md
5. **重新确认**：展示修改后的计划摘要，等待用户再次确认
6. **重新推进**：确认后按 PDEVI 流程继续（若 Design 受影响，需重新审视 design.md）

> **关键**：修改计划时 PjM **必须先更新 resume.md 状态**再分派工作，确保所有 Agent 读到一致的阶段信息。不可在 resume.md 未回退的情况下直接修改 proposal.md 并继续推进。
