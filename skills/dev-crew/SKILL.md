...
name: crew
user-invocable: true
description: AI team collaboration protocol — multi-agent orchestration with PDEVI workflow, persistent memory, and checkpoint-based quality control. Turns any AI into a coordinated dev team.
metadata:
  author: lordmos
  version: 0.5.0
---
...
若当前项目尚未初始化 DevCrew 工作区，先执行初始化：

```bash
npx @lordmos/dev-crew init
```

> 若 CLI 不可用，请手动创建下方「文件系统」章节描述的目录结构。
