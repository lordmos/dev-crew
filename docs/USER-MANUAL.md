...
> DevCrew 内建了一个完整的软件开发团队（项目经理、产品经理、架构师、开发、测试、审查等），按需组建，自动分工协作。你不需要手动分配角色——团队在幕后自动编排。你只管说做什么。

---

## 5 个 Skills

| Skill | CLI | MCP Tool | 什么时候用 |
|-------|-----|----------|-----------|
| **init** | `crew init` | `crew_init` | 第一次使用 |
| **plan** | `crew plan <名称>` | `crew_plan` | 有新任务 |
| **status** | `crew status` | `crew_status` | 随时 |
| **release** | `crew release` | `crew_release` | 任务做完 |
| **agents** | `crew agents` | `crew_agents` | 查看可用专家 |

> 你也可以用自然语言，AI 同样能理解。"帮我看看进度"，AI 自动调用 status skill。
